...
<template>
  <div class="fixed inset-0 z-[110] flex items-center justify-center bg-n-slate-12/30 backdrop-blur-[2px]">
    <div class="bg-white rounded-3xl w-full max-w-md shadow-2xl border border-n-weak overflow-hidden flex flex-col max-h-[60vh]">
      <header class="p-5 border-b border-n-weak flex justify-between items-center bg-n-slate-1">
      ...
      <div class="p-4 border-b border-n-weak">
        <div class="relative">
          <span class="absolute left-3 top-1/2 -translate-y-1/2 i-lucide-search size-4 text-n-slate-9" />
          <input
            v-model="searchQuery"
            placeholder="Buscar contato (nome, e-mail...)"
            class="w-full pl-10 pr-4 py-2 bg-n-slate-1 border border-n-weak rounded-xl focus:border-n-brand focus:ring-1 focus:ring-n-brand outline-none text-sm transition-all"
            autofocus
          />
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-2">
        ...
        <button
          ...
          class="w-full flex items-center gap-3 p-3 hover:bg-n-alpha-1 rounded-xl transition-colors text-left group"
        >
          <div class="size-10 rounded-full bg-n-brand/10 flex items-center justify-center text-n-brand font-bold text-sm">
            {{ (contact.name || 'U').charAt(0).toUpperCase() }}
          </div>
          <div class="flex-1 min-w-0">
            <h4 class="font-bold text-n-slate-12 text-sm truncate group-hover:text-n-brand transition-colors">
      ...
      <footer class="p-4 bg-n-slate-1 border-t border-n-weak text-center">
